...
Graph newGraph(int n) {
    ...
    G->neighborV = calloc(n + 1, sizeof(GraphObj));
    G->vertex = n;
...
void addArc(Graph G, int u, int v) {
    ...
    moveTo(G->neighborV[u], 0); // set the cursor of list at the neighbor index u to 0 
...
void BFS(Graph G, int s) {
    ...
    while (length(A) > 0) { // while the length of the list is greater than 0
        ...
        moveTo(G->neighborV[x], 0); // set the list at index of the neighbor x to 0 
        while (index(G->neighborV[x]) != -1) { // while the cursor is defined 
            ...
            if (G->color[y] == WHITE) { // if the vertex is "as of yet undiscovered"
                ...
                append(A, y); // append the parent vertex to the front of the list
            }
